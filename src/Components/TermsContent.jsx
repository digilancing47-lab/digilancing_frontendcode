  ...
  const sections = [
    ...
    },
    {
      ...
      content: [
        ...
        "33.3 Anti-Poaching and Harassment",
    ...
    },
    {
      ...
      content: [
        ...
        "35.2 Commission Caps",
        "Earnings are capped based on the selected affiliate package.",
        "35.3 Revisions to Structure",
  ...
  return (
    <div className="space-y-6 text-gray-800 leading-relaxed text-sm">
      <header className="text-center mb-6">
        <h1 className="text-xl font-bold uppercase mb-2 text-gray-700">
          TERMS & CONDITIONS
        </h1>
        <h2 className="text-lg font-normal uppercase text-gray-500">
          Digilancing Private Limited — Effective Date: {effectiveDate}
        </h2>
      </header>

      {sections.map((section, idx) => (
        <section key={section.id || idx} className="space-y-3">
          <h2 className="text-lg font-semibold text-blue-700 border-l-4 border-blue-700 pl-3">
            {section.title}
          </h2>
          ...
          {section.subSections &&
            section.subSections.map((sub, sidx) => (
              <div key={sidx} className="mt-3 ml-2">
                <h3 className="text-sm font-semibold text-gray-800 mt-3 mb-2">{sub.subtitle}</h3>
                {sub.content &&
